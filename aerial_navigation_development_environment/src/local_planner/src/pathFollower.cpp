...
double stopRotYaw1 = 90.0; // this is the angle at which the vehicle will stop
double stopRotYaw2 = 10.0; // this is the angle at which the vehicle will start to turn
double stopDis = 0.5;       // this is the distance at which the vehicle will stop in front of the goal
double slowDis = 2.0;
...
void joystickHandler(const sensor_msgs::Joy::ConstPtr& joy)
{
  ...
  if (joy->axes[2] >= -0.1 || joy->axes[5] < -0.1) { // if LT not pressed and RT Pressed 
    joyFwd = joy->axes[4];
  ...
  if (joy->axes[5] < -0.1) {  // if RT pressed 
    manualMode = true;
    autonomyMode = false;
    autoAdjustMode = false;
  } else {
    ...
    if (joy->axes[2] < -0.1) { // if LT pressed
