...
float searchRadiusHori = 3.15; // 3.15
...
// These are used for storing joystick inputs which are scaled by... 
float joyFwd = 0;   // not scaled 
float joyLeft = 0;  // yawDiffLimit 
float joyUp = 0;    // pitchDiffLimit

// These are the control modes
bool manualMode = true;       // Manual Mode
bool autonomyMode = false;    // Waypoint mode
bool autoAdjustMode = false;  // Smart Joystick Mode

int systemInitDelay = 5;
int stateInitDelay = 100;

//creates smart pointers called laserCloud, laserCloudCrop, laserCloudDwz, etc. 
...
// this takes the data from the state estimation topic and updates the odomTime, odomRoll, odomPitch, odomYaw, odomX, odomY, odomZ variables.
// It looks like the state estimation data is being offset by the trackingCamScale, trackingCamXOffset, trackingCamYOffset, trackingCamYOffset vars
// so that the MAV following camera can be positioned correctly in the UE world
// goalX, goalY, goalZ is also being incremented based on the state estimation pose/position data scaled by trackingCamScale
...
// This updates trackRoll, trackRoll, trackPitch, trackYaw, trackX, trackY, trackZ with the odometry information from the /track_point_odom topic
// /pathFollower publishes to the /track_point_odom topic
...
// [COMMENTED joystickHandler] 
void joystickHandler(const sensor_msgs::Joy::ConstPtr& joy)
{
  
  bool LT_PRESSED = joy->axes[2] < -0.01 ? true : false;
  bool RT_PRESSED = joy->axes[5] < -0.01 ? true : false;
  ROS_INFO("\nLT_PRESSED: %d\nRT_PRESSED: %d\nmanualMode: %d\nautonomyMode: %d\nautoAdjustMode: %d", LT_PRESSED, RT_PRESSED, manualMode, autonomyMode, autoAdjustMode);
  
  if ((RT_PRESSED && !LT_PRESSED) || (RT_PRESSED && LT_PRESSED)){ //manual mode
    manualMode = true;
    autonomyMode = false;
    autoAdjustMode = false;
    
    joyFwd = joy->axes[4];                            //  assign right stick up/down axis to joyFwd
    joyLeft = yawDiffLimit * joy->axes[3];            //  assign scaled right stick left/right to joyLeft
    joyUp = pitchDiffLimit * joy->axes[1];            //  assign scaled left stick up down to joyUp
  
  } else if (!RT_PRESSED && !LT_PRESSED){ // auto adjust mode
    manualMode = false;
    autonomyMode = true;
    autoAdjustMode = true;
    joyFwd = joy->axes[4];                            //  assign right stick up/down axis to joyFwd
  ...
  } else if (!RT_PRESSED && LT_PRESSED){ // autonomy mode 
    manualMode = false;
    autonomyMode = true;
    autoAdjustMode = false;
    joyFwd = 20.0;
    ...
    ROS_INFO("This Should Not Happen!!!!");
  }

  // if right button is pressed then clear the stored point clouds in laserCloudKeep and laserCloudKeepDwz (not sure what these are as yet)
...
{
  // If AutonomyMode is set, based on the float received on /auto_mode, set or clear autoAdjustMode 
  // Then set joyFwd to 0 if float received is < 0 otherwise set joyFwd to 1   
  if (autonomyMode) { 
    if (autoMode->data < -0.5) {  // this topic contains messages of type F32 called data
      autoAdjustMode = true;      // This is assumed to set the flag to implement obstacle avoidance (1/2 parts of smart joystick mode )
    } else {
      autoAdjustMode = false;
    }
...
int main(int argc, char** argv)
{
  ...
  nhPrivate.getParam("stateEstimationTopic", stateEstimationTopic); // Set in AirSim Launch File
  nhPrivate.getParam("autonomyMode", autonomyMode);                 // Set in local_planner_outdoor.launch OR local_planner_indoor.launch 
  nhPrivate.getParam("depthCloudTopic", depthCloudTopic);           // Set in AirSim Launch File
  nhPrivate.getParam("depthCloudDelay", depthCloudDelay);           // Set in AirSim Launch File
  nhPrivate.getParam("depthCamPitchOffset", depthCamPitchOffset);   // Set in AirSim Launch File
  nhPrivate.getParam("depthCamXOffset", depthCamXOffset);           // Set in AirSim Launch File
  nhPrivate.getParam("depthCamYOffset", depthCamYOffset);           // Set in AirSim Launch File
  nhPrivate.getParam("depthCamZOffset", depthCamZOffset);           // Set in AirSim Launch File
  nhPrivate.getParam("trackingCamBackward", trackingCamBackward);   // Set in AirSim Launch File
  nhPrivate.getParam("trackingCamXOffset", trackingCamXOffset);     // Set in AirSim Launch File
  nhPrivate.getParam("trackingCamYOffset", trackingCamYOffset);     // Set in AirSim Launch File
  nhPrivate.getParam("trackingCamZOffset", trackingCamZOffset);     // Set in AirSim Launch File
  nhPrivate.getParam("trackingCamScale", trackingCamScale);         // Set in AirSim Launch File
  ...
  // This adjusts goal Z position to be LIMITED by the max elevation or height (30) passed in the lanuchfile  
  if (goalZ > maxElev) goalZ = maxElev;

  // This sets manual mode off by and joyFwd to 1 if autonomyMode is true 
  if (autonomyMode) {
    manualMode = false;
    joyFwd = 1.0;
  }
  ...
  // SUBSCRIBERS
  // sets up the state estimation subscription 
  // /vehicleSimulator and /pathFollower publish to this topic
  ros::Subscriber subStateEstimation = nh.subscribe<nav_msgs::Odometry> (stateEstimationTopic, 5, stateEstimationHandler);

  // sets up the depthCloudTopic subscription
  // /standalone_nodelet publishes to this topic
  ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2> (depthCloudTopic, 5, laserCloudHandler);

  // sets up /track_point_odom subscription
  // 
  ...
  // auto_mode subscription {/PathFollower node publishes to auto_mode topic}
